...
const ModernProfile = () => {
  ...
  const [widget, setWidget] = useState('wallet');
    ...
    const fetchAllData = async () => {
      try {
        ...
        const { data: servicesData, error: servicesError } = await Fetch({
          url: `${import.meta.env.VITE_API_SERVICE_URL}/service/artisan/${
            storedUser?.id || storedUser?._id
          }`,
          token: state.token,
        });
  ...
  const filteredOrders = Array.isArray(orders)
    ? orders.filter((order) => order.artisanId === userData.id)
    : [];
  ...
  const toggleWidget = (mode) => {
    if (mode === 'wallet') {
      setWidget('availability');
    } else {
      setWidget('wallet');
    }
  };
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-8 mb-8">
            <div className="flex flex-col lg:flex-row gap-8 items-stretch">
              ...
              {widget === 'wallet' && (
                <div className="flex items-center gap-3 lg:gap-4">
                  ...
                  <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-lg border border-gray-200 p-6 flex flex-col items-center justify-center w-56 hover:shadow-xl transition-shadow">
                    <Wallet size={28} className="text-green-500 mb-3" />
                    <span className="text-lg font-semibold text-gray-900">
                      ₦{wallet?.balance?.toLocaleString() || '0'}
                    </span>
                    <button
                      onClick={() => setWithdrawWalletModalOpen(true)}
                      className="mt-4 px-4 py-2 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-xl font-medium shadow-md hover:shadow-lg hover:scale-105 transition-all flex items-center gap-2"
                    >
              ...
              {widget === 'availability' && (
                <div className="flex items-center gap-3 lg:gap-4">
                  <div className="lg:w-80 xl:w-96 bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                      ...
                      <div>
                        ...
                        {businessHoursOpen && userData.businessHours && (
                          <div className="space-y-3 pl-6">
                            {Object.entries(userData.businessHours).map(
                              ([day, hours]) => (
                                <div
                                  key={day}
            ...
            <div className="lg:col-span-2">
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
                ...
                <p className="text-gray-700 leading-relaxed mb-6">
                  Professional artisan specializing in{' '}
                  {userData.skillSet?.join(', ').toLowerCase()}. Based in{' '}
                  {userData.location}, I provide high-quality services through{' '}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {userData.phoneNumber || '+234 904454667'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border mb-8 border-white/20 p-6">
            ...
            {loading.orders ? (
              ...
              <div className="grid gap-6">
                {filteredOrders.map((order) => {
                  ...
                  return (
                    <div
                      key={order.id}
                      className="border border-gray-200 rounded-2xl p-6 hover:shadow-lg transition-all duration-300 bg-gradient-to-r from-white to-gray-50/50"
                    >
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                            <div>
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <span
                                  className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium border ${statusDisplay.color}`}
                                >
                                  {statusDisplay.icon}
                                  {statusDisplay.text}
                                </span>
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-sm text-gray-600">
                                Escrow:{' '}
                                {order.escrowStatus?.replace('_', ' ') || 'N/A'}
                              </div>
                            </div>
                          </div>
              ...
              <div className="flex flex-col items-center justify-center text-center text-gray-600 py-12 px-4 bg-white/70 rounded-2xl">
                ...
                <p className="text-sm text-gray-500">
                  You haven't received any orders yet. When you do, they'll show
                  up here.
                </p>
              </div>
            )}
