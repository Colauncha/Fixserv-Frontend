...
const SelectCampaignModal = ({ isOpen = true, parentEmail, onClose = () => {} }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_NEWS_LETTER_URL}/api/subscribers`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#7A9DF7] to-[#7A9Dd7] py-4 px-6 text-white relative">
          <button
            onClick={onClose}
          ...
          {isSuccess ? (
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                className="cursor-pointer w-full bg-gradient-to-r from-[#7A9DF7] to-[#7A9Dd7] text-white py-3 px-6 rounded-lg font-medium hover:from-[#7895c5] hover:to-[#618cfa] focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {isLoading ? (
