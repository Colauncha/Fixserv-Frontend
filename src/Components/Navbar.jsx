  ...
  const handleSignUp = () => {
    ...
    return (
      ...
      <nav className="flex justify-between items-center pt-10 w-[80%] h-16 max-w-7xl bg-white rounded-md">
        ...
        <button 
        onClick={handleLogIn}
        className="bg-[#779BE7] text-white px-4 py-1 h-10 w-25 text-md gap-20 rounded-lg">Log in</button>

        <button
        onClick={handleSignUp}
        className="bg-[#779BE7] text-white px-4 py-1 h-10 w-25 text-md gap-20 rounded-lg">Sign Up</button>
        </div>
      </nav>
      </div>
    );
