...
const AdminDashboard = () => {
    ...
    try {
      ...
      const fetchRecentActivities = async () => {
        try {
          ...
          const activities =
            data?.data?.activities || data?.data || data?.activities || [];
  ...
  const getBarHeight = (value) => {
    ...
    const maxValue = Math.max(
      ...dashboardData.transactionVolume.map((item) => item.total),
      1,
    );
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setDashboardData({
        totalUsers: usersData?.data?.totalUsers?.total || 0,

        activeArtisans: usersData?.data?.totalUsers?.artisans || 0,

        totalClients: usersData?.data?.totalUsers?.clients || 0,

        totalAdmins: usersData?.data?.totalUsers?.admins || 0,

        newSignups: usersData?.data?.newSignups?.total || 0,

        gmv: ordersData?.data?.gmv?.thisPeriod?.total || 0,

        transactionsToday: ordersData?.data?.transactions?.total || 0,

        openDisputes: disputesData?.data?.open || 0,

        transactionVolume: ordersData?.data?.revenueBreakdown?.weekly || [],
      });

      setTopArtisans(topArtisansData?.data || []);
    } catch (error) {
  ...
  const handleOpenCard = async (type) => {
    try {
      ...
      switch (type) {
        case "USERS":
          endpoint = "/api/admin/dashboard/users?period=month&page=1&limit=20";
      ...
      console.log("FULL USER RESPONSE =>", JSON.stringify(data, null, 2));

      console.log(data);

      const artisans = (data?.data?.activeArtisans?.artisans || []).map(
        (user) => ({
          ...user,
          role: "ARTISAN",
        }),
      ...
      const clients = (data?.data?.activeClients?.clients || []).map(
        (user) => ({
          ...user,
          role: "CLIENT",
        }),
      ...
      const adminsCount = data?.data?.totalUsers?.admins || 0;

      const admins = Array.from({ length: adminsCount }, (_, index) => ({
        id: `admin-${index + 1}`,
      ...
      const allUsers = [...artisans, ...clients, ...admins];
      ...
      if (type === "USERS") {
        ...
        setUsersList(data?.data?.newSignups?.users || []);
      } else {
        setUsersList([]);
      }
    } catch (error) {
            ...
            <div className="flex-1 p-6 overflow-hidden">
              ...
              <div className="mb-4">
                ...
                >
                  ...
                  <option value="ARTISAN">All Artisans</option>
          ...
          >
            ...
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
              <div>
                ...
                <p className="text-sm text-gray-500">Manage and view users</p>
              </div>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm font-medium">
                  {selectedUser.role || selectedUser.userType || "CLIENT"}
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
              {[
                ...
                >
                  ...
                  <p
                    className={`text-xs ${card.danger ? "text-red-500" : "text-[#3E83C4]"}`}
                  >
                    {card.note}
                  </p>
                </div>
