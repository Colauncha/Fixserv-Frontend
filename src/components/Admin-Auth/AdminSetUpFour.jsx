...
import upload from "../../assets/Admin Images/four.png";
import { useNavigate } from "react-router-dom";



const AdminSetUpFour = () => {
        ...
        <button onClick={() => navigate("/admin")} className="absolute top-6 cursor-pointer right-6 text-sm border border-gray-300 px-3 py-1 rounded text-gray-600">
          Skip onboarding
              ...
              <p className="text-center text-sm text-gray-600 max-w-md mx-auto mb-10">
               Track user growth, booking trends, payment metrics, and overall platform performance with detailed analytics and reports.
              </p>
            ...
            <button onClick={() => navigate("/admin")}  className="border border-gray-300 cursor-pointer text-gray-600 text-sm px-5 py-2 rounded">
              Previous
            </button>

            

                <button onClick={() => navigate("/admin")} className="bg-[#3E83C4] text-white text-sm px-6 py-2 rounded cursor-pointer">
                  Get Started
                </button>
              </div>
              </div>
