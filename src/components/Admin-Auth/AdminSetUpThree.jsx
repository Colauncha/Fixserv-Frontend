...
import upload from "../../assets/Admin Images/three.png";
import { useNavigate } from "react-router-dom";



const AdminSetUpThree = () => {
        ...
        <button onClick={() => navigate("/admin")} className="absolute top-6 cursor-pointer right-6 text-sm border border-gray-300 px-3 py-1 rounded text-gray-600">
          Skip onboarding
              ...
              <p className="text-center text-sm text-gray-600 max-w-md mx-auto mb-10">
               Monitor all platform transactions in real-time, resolve disputes efficiently, and handle escalations with comprehensive tools.
              </p>
            ...
            <button onClick={() => navigate("/admin/admin-setup-two")}  className="border border-gray-300 cursor-pointer text-gray-600 text-sm px-5 py-2 rounded">
              Previous
            </button>

            

                <button onClick={() => navigate("/admin/admin-setup-four")} className="bg-[#3E83C4] text-white text-sm px-6 py-2 rounded cursor-pointer">
                  Next
                </button>
              </div>
              </div>
...
export default AdminSetUpThree;
