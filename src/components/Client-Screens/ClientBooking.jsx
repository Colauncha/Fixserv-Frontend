...
//       const res = await fetch(
//         `/service-api/api/service/artisan/${artisanId}`,
//         {
//           headers: token ? { Authorization: `Bearer ${token}` } : undefined,
//         }
//       );
...
//       } catch {
//         console.log("SERVICE RAW RESPONSE:", text);
//         throw new Error(`Service API returned non-JSON (status ${res.status})`);
//       }
...
// const getClientId = () => {
//   const raw = localStorage.getItem("fixserv_user");
//   if (!raw) return null;
//   try {
//     const u = JSON.parse(raw);
//     return u?.id || u?._id || u?.user?.id || u?.data?.id || null;
//   } catch {
//     return null;
//   }
// };
...
//   artisanData?.roleTitle ||
//   artisanData?.categoryTitle ||
//   "Technician";
...
//     if (!clientId) throw new Error("No clientId found. Please login again.");

//     // ✅ 1) Upload product (FormData)
//     const fd = new FormData();
//     fd.append("objectName", `${bookingData.deviceType} - ${bookingData.brand} ${bookingData.model}`.trim());
//     fd.append("description", bookingData.issueDescription || "");
//     fd.append("productImage", bookingData.imageFile);

//     const uploadRes = await fetch(
//       `/api/upload/${clientId}/upload-products`,
//       {
//         method: "POST",
...
//     if (!uploadRes.ok) {
//       throw new Error(uploadJson?.message || "Failed to upload product");
//     }

//     const uploadedProductId =
//       uploadJson?.product?._id ||
//       uploadJson?.product?.id ||
//       uploadJson?.uploadedProductId;

//     if (!uploadedProductId) {
//       console.log("UPLOAD RESPONSE:", uploadJson);
//       throw new Error("Upload succeeded but uploadedProductId not found");
//     }
...
//       },
//       body: JSON.stringify({
//         serviceId: bookingData.serviceRequired, // ✅ serviceId
//         uploadedProductId,
//       }),
//     });
...
//     // ✅ Success: go to summary (you can also go to a success page)
...
//     {/* ✅ NOT hardcoded */}
//     <p className="text-sm text-[#656565]">{professionLabel}</p>

//     {/* ✅ rating + reviewsCount (NOT artisanData.reviews) */}
...
//       {(Array.isArray(artisanData.categories) && artisanData.categories.length
...
//       {/* ✅ experience from createdAt (NOT hardcoded 5+ years) */}
//       <p>
//         Experience:{" "}
...
//         Location:{" "}
//         <span className="font-medium text-black">
//           {artisanData.location?.trim() ? artisanData.location : "Unknown"}
//         </span>
//       </p>
//     </div>
//   </div>
