...
const WALLET_BASE = import.meta.env.VITE_WALLET_API_BASE_URL;
...
console.log("WALLET_BASE =>", import.meta.env.VITE_WALLET_API_BASE_URL);
...
const handleSendInvite = async () => {
  ...
  try {
    ...
    const url = `${WALLET_BASE}/referral/send-invite`;
    ...
    if (!res.ok) {
      throw new Error(data?.message || "Unable to send invite, please try again or contact support if the issue persists.");
    }
  ...
  const fetchFixpoints = async () => {
    try {
      ...
      const res = await fetch(
        `${WALLET_BASE}/fixpoints/balance/${userId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (data?.success) {
        ...
        setFixpoints(0); // ✅ fallback
      }
    } catch (error) {
      console.log("Fixpoints fetch error:", error);
      setFixpoints(0); // ✅ safety
    }
  };
...
useEffect(() => {
  const fetchReferralInfo = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${WALLET_BASE}/referral/info/${userId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
