...
const DRAFT_ENDPOINT = import.meta.env.DEV
  ? "/api/orders/draft"
  : `${import.meta.env.VITE_ORDER_API_BASE_URL.replace(/\/$/, "")}/draft`;
...
const RequestRepair = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!deviceType.trim() || !deviceBrand.trim() || !deviceModel.trim()) {
      setError("Device type, device brand, and device model are required.");
      return;
    }
    ...
    if (!file) {
      setError("Please upload an image of the damaged device.");
      return;
    }
    ...
    try {
      ...
      const { uploadedProductId } = await uploadProductImageFile({
        userId,
        file,
        objectName: objectName.trim() || deviceType.trim() || "Device",
        description: description.trim() || "Device repair request",
      });
      ...
      navigate("/client/technician", {
        state: {
          draftOrderId: draftOrderIdStr,
          uploadedProductId,
          deviceType: draftData?.deviceType || deviceType.trim(),
          deviceBrand: deviceBrand.trim(),
          deviceModel: deviceModel.trim(),
          serviceRequired: serviceRequired.trim(),
          description: description.trim(),
      ...
      if (
        ...
        setError(
          getProductUploadFriendlyError(
            err,
            "Unable to upload the device image. Please try again."
          )
        );
      } else {
        setError(
          getUserFriendlyError(err, "Unable to create request. Please try again.")
        );
      }
    } finally {
      setLoading(false);
        ...
        <div className="max-w-7xl mx-auto px-2 md:px-6">
          ...
          <div className="max-w-3xl space-y-5">
            ...
            <div>
              ...
              <label className="border-2 border-dashed border-gray-300 rounded-xl h-44 flex flex-col items-center justify-center cursor-pointer transition hover:border-blue-400 overflow-hidden">
                {preview ? (
                  <img
                    src={preview}
